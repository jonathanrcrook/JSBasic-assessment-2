...
function removeThings(arr, num) {
	var filtered = arr.filter( function(str, idx) {
		return str !== num;
	})
	return filtered
}
...
// #14 Create a function called spiderTest that takes in an object that is an actor, it looks to see if the actor's name is Andrew Garfield
// and changes the actor object to have a property called spiderRating with a value of 10.
// If their name is Tobey Maguire set spiderRating to 0.  otherwise set spiderRating to 'not a spiderman'.
...
*/
	...
	function whatIsItOutside(temp, hum, cloudiness) {
		if (temp > 85 && hum > 45) {
			return "I'm all sweat"
		} else if (temp < 40 && cloudiness > 60) {
			return "I have icicles"
		} else if (temp > 85 && hum < 45 && cloudiness < 20) {
			return "I'm literally in the desert"
		} else if (temp > 90 || hum > 55 || cloudiness > 65) {
